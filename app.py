...
def create_app():
    ...
    def month_summary(year, month):
        ...
        ).filter(DayRecord.date.between(first_day, last_day_date)).one()

        data = totals._asdict()
        data['expenses'] = sum([data[k] for k in ('sugar', 'essence', 'tax', 'water',
    ...
    def year_summary(year):
        totals = db.session.query(
            ...
            func.coalesce(func.sum(DayRecord.other), 0).label('other'),
            func.coalesce(func.sum(DayRecord.revenue), 0).label('revenue')
        ).filter(func.strftime('%Y', DayRecord.date) == str(year)).one()
